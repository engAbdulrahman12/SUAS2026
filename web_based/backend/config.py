...
# its background thread never races the main connection's recv_match()
# calls. SITL exposes 3 spare TCP ports for exactly this; MAVProxy gets a
# 3rd --out for the real drone.
SITL_STATUS_URI  = "tcp:127.0.0.1:5763"    # SITL's 3rd spare TCP port
DRONE_STATUS_URI = "udp:0.0.0.0:14553"     # 3rd MAVProxy --out

def default_status_uri():
    return SITL_STATUS_URI if TEST_FLAG else DRONE_STATUS_URI
